C     (*** August 2005 version ***)
      IMPLICIT REAL*8 (A-H,O-Z)
      DIMENSION AZ(1000000),TYPE(200),U(200,20),IA(200,6),IU(200,0:20)  INT00030
      CHARACTER TYPE*5
...
C     ////////////////////////////////////////////////////////////      INT14360
     ...
     $        XA,XMASS,S,SS,B,BS,A,UGF,XS,XT,XL,XKI,XAR,
     $        Z,TYPE,U,IA,IU,NCORE,IFLAG)
      ...
      DIMENSION A(NC,NC),UGF(NC,1),BS(NC,NC),XS(NC,NC),XL(NC,1)
...
C        COMPLETE THE V(M,N,P,Q) COMPUTATION.
 ...
 600  NFREQ=IABS(NFREQ)
     ...
     $           XMASS,XA,S,BS,UGF,A,XS,XT,XS,XL,IFLAG)
...
CSA
 ...
 134  F2(N,M)=F2(M,N)                                                   INT15570
      ...
      READ(INP3,6) M,N                                                  INT15630
      IF(M.NE.NA.OR.N.NE.NNC) THEN                                      INT15640
           IFLAG=3                                                      INT15650
           RETURN                                                       INT15660
      END IF                                                            INT15670
 ...
 152  CONTINUE                                                          INT16880
      ...
      READ(INP4,6) M,N                                                  INT16960
      IF(M.NE.NA.OR.N.NE.NNC) THEN                                      INT16970
           IFLAG=4                                                      INT16980
           RETURN                                                       INT16990
      END IF                                                            INT17000
...
C////////////////////////////////////////////////////////////////////// INT47140
     ...
     $                       XMASS,XA,W,BS,F,XR,XS,XT,XU,XL,IFLAG)      INT47160
      IMPLICIT REAL*8 (A-H,O-Z)                                         INT47170
      LOGICAL ITEST                                                     INT47180
      DIMENSION XA(NAD,3),BS(NC,NC),F(NC,NC),XS(NC,NC),XT(NC,NC)        INT47190
      DIMENSION XMASS(NA),W(NSX),XR(NC,1),XU(1),IA(5,NSX),XL(NC,2)      INT47200
      DIMENSION V1(3),V2(3),V3(3),ZETA(3)
      COMMON /IO/ IIN1,IOUT,IIN2,ICHECK,NPRT,
     $   I11,I15,I17,I20,I24,
     $   I12,I16,I18,I21,I25,
     $   I31,I32,I33,I35,I36,I37,
...
C     CINT=                                                             INT47270
  ...
  36  FORMAT(6F12.8)
  37  FORMAT(//,3X,'CORIOLIS CONSTANTS'/)
  38  FORMAT(/,' X AXIS'/)
  39  FORMAT(/,' Y AXIS'/)
  40  FORMAT(/,' Z AXIS'/)
CSA
...
C
CWA  August 2005
      IF(NFREQ.EQ.6.OR.NFREQ.EQ.8.OR.NFREQ.EQ.9) THEN 
      WRITE(IOUT,32)
      REWIND I12
      READ(I12,33) NAX
 ...
 501  XL(I,1)=W(I)
      ...
      IF(NFREQ.EQ.9) THEN
      READ(IIN1,4) JF1MAX
      WRITE(IOUT,50) JF1MAX
  50  FORMAT(/,1X,'DISTINGUISHED REACTION COORDINATE = ',I5,/)    
      K1=JF1MAX-1
      K2=JF1MAX+1
      DO 502  I=1,K1
      DO 502  J=1,K1
 ...
 506    XR(I,J-1)=F(I,J)
      DO 508  I=K2,NSX
      DO 508  J=K2,NSX
 508    XR(I-1,J-1)=F(I,J)
      DO 510  I=1,K1
 510    XR(I,NSX)=F(I,JF1MAX)
      DO 512  I=K2,NSX   
 512    XR(I-1,NSX)=F(I,JF1MAX)
      CALL FLIN(XR,NC,NSX-1,1,DET)
      DO 513  I=1,NSX-1
 513    W(I)=-XR(I,NSX)
      TAU=ZERO
      DO 514  I=1,K1
 514    TAU=TAU-W(I)*F(I,JF1MAX)
      DO 516  I=K2,NSX
 516    TAU=TAU-W(I-1)*F(I,JF1MAX)
      WRITE(IOUT,52) TAU 
  52  FORMAT(/,1X,'DRP PROJECTED F(TAU,TAU) = ',F16.12,/)
      F(JF1MAX,JF1MAX)=TAU
      END IF
CWA
C
...
CWA   August 2005
      IF(NFREQ.EQ.9) THEN
       DO 520  I=1,K1
          XU(I)=ZERO
       DO 522  J=1,K1
 522      XU(I)=XU(I)+W(J)*XR(I,J)
       DO 524  J=K2,NSX
 524      XU(I)=XU(I)+W(J-1)*XR(I,J)
 520   CONTINUE
       DO 530  I=K2-1,NSX-1
          XU(I)=ZERO
       DO 532  J=1,K1
 532      XU(I)=XU(I)+W(J)*XR(I+1,J)
       DO 534  J=K2,NSX
 534      XU(I)=XU(I)+W(J-1)*XR(I+1,J)
 530   CONTINUE
       DO 540  I=1,K1
 540      XU(I)=XU(I)+TWO*XR(I,JF1MAX) 
       DO 542  I=K2-1,NSX-1
 542      XU(I)=XU(I)+TWO*XR(I+1,JF1MAX) 
       GTAU=XR(JF1MAX,JF1MAX)
       DO 550  I=1,NSX-1
 550      GTAU=GTAU+XU(I)*W(I)
       WRITE(IOUT,53) GTAU
  53   FORMAT(/,1X,'DRP PROJECTED G(TAU,TAU) = ',F16.12,/)
       END IF
C
...
Cwa
 157  CALL RSP(NC,NSX,NV,XU,W,1,XT,XL(1,1),XL(1,2))                     INT48350
...
Cwa
      ...
      KMX=1
      DO 167  M=2,NSX
      IF(DABS(XT(M,N)).GT.XMX) THEN
        XMX=DABS(XT(M,N)) 
        KMX=M
      END IF
 167  CONTINUE
      IF(XT(KMX,N).LT.ZERO) THEN
      DO 169  M=1,NSX
 169  XT(M,N)=-XT(M,N)
      END IF
 166  CONTINUE
...
C
Cwa  XR = G(1/2), XS = Ls 
Cwa  Form Lx tensor into XT 
        DO 400  N=1,NSX
        DO 400  M=1,NSX
             XT(M,N+NSX)=XT(M,N)
 400         XT(M,N)=XR(M,N)
        CALL FLIN(XT,NC,NSX,NSX,DET)
        DO 402  I=1,NC
        II=(I-1)/3+1
        DO 402  J=1,NSX
        XT(I,J)=ZERO 
        DO 403  K=1,NSX
 403    XT(I,J)=XT(I,J)+BS(K,I)*XT(K,J+NSX)         
 402    XT(I,J)=XT(I,J)/DSQRT(XMASS(II))
Cwa     CALL TABLE2(NC,NC,NSX,W,XT) 
Cwa  Form Coriolis matrices       
        DO 410  M=1,NSX
        DO 410  N=1,NSX
        XL(M,N)=ZERO 
        XL(M,N+NC)=ZERO
 ...
 405    ZETA(K)=ZERO
        ...
        DO 407  K=1,3
        V1(K)=XT(3*I-3+K,M) 
 407    V2(K)=XT(3*I-3+K,N)
        CALL VECPRO(V1,V2,V3)
        DO 408  K=1,3
 408    ZETA(K)=ZETA(K)+V3(K)
 406    CONTINUE        
        XL(M,N)=XL(M,N)+ZETA(1) 
        XL(M,N+NC)=XL(M,N+NC)+ZETA(2)
 410    XL(M,N+2*NC)=XL(M,N+2*NC)+ZETA(3)
 ...
 412    XT(M,N)=XL(M,N)
        ...
        DO 414  M=1,NSX
        DO 414  N=1,NSX
 414    XT(M,N)=XL(M,N+NC)
        CALL TABLE3(NC,NSX,W,XT) 
        WRITE(IOUT,40)
        DO 416  M=1,NSX
        DO 416  N=1,NSX
 416    XT(M,N)=XL(M,N+2*NC)
        CALL TABLE3(NC,NSX,W,XT) 
        END IF
Cwa
        IF(NMODE.EQ.1) THEN
